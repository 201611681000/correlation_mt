...
def cal_bleu (samples, task, args):
    ...
    for i in range (len(target_tensor)):
        tgt_tokens = utils.strip_pad(target_tensor[i],tgt_dict.pad()).int().cpu()
        cand_tokens = utils.strip_pad(cand_tensor[i],tgt_dict.pad()).int().cpu()

        tgt_str = tgt_dict.string(tgt_tokens, None, escape_unk=True, extra_symbols_to_ignore={tgt_dict.eos()})
        tgt_str = decode_fn(tgt_str)

        cand_str = tgt_dict.string(cand_tokens, None, escape_unk=True, extra_symbols_to_ignore={tgt_dict.eos()})
        cand_str = decode_fn(cand_str)

        bleuscore = sacrebleu.corpus_bleu([cand_str],[[tgt_str]],use_effective_order=True)
...
def train(args, trainer, task, epoch_itr):
    ...
    for i, samples in enumerate(progress):
        with metrics.aggregate("train_inner"), torch.autograd.profiler.record_function(
            ...
            samples = [cal_bleu (samples[0], task, args)]
    ...
    for subset in subsets:
        ...
        with metrics.aggregate(new_root=True) as agg:
            for sample in progress:
                sample = cal_bleu (sample, task, args)
